...
antipico1[:,1] /= 10
antipico2[:,2] *= 20 * 10**-6 / 10
antipico2[:,2] /= 10
#mergeo los datos
tabla = np.concatenate((antipico1, antipico2), axis = 0)
...
popt, pcov = curve_fit(Transf, tabla[:,0], T, p0 = 10**-12)
print (popt)
dominio = np.arange(min(tabla[:,0]),max(tabla[:,0]),0.5) 
plt.plot(dominio, Transf(dominio, popt))
plt.plot(tabla[:,0], T, marker = 'x', linestyle = 'none', color = 'r')
plt.show()
